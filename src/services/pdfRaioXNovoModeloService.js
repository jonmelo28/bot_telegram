...
function gerarPdfRaioXNovoModelo({ usuario, dados }) {
  return new Promise((resolve, reject) => {
    ...
    const detalhesSecao = Array.isArray(dados.detalhesSecao) ? dados.detalhesSecao : [];
    ...
    function montarTotaisGenerico(detalhes, campoMeta, campoVenda, campoFat, campoMetaPos, campoRealPos, campoFatPos) {
  return detalhes.reduce(
    (acc, item) => {
      acc.metaValor += Number(item[campoMeta] || 0);
      acc.vlVendido += Number(item[campoVenda] || 0);
      acc.vlFaturado += Number(item[campoFat] || 0);
      acc.metaPos += Number(item[campoMetaPos] || 0);
      acc.realPos += Number(item[campoRealPos] || 0);
      acc.fatPos += Number(item[campoFatPos] || 0);
      return acc;
    },
    {
      metaValor: 0,
...
function montarFaixasPositivacao(metaPos) {
  ...
  return {
    ...
    // REALIZADO POR SEÇÃO
    const rowsDetalhes = detalhesSecao.map((item) => {
    ...
    const totalSecao = montarTotaisSecao(detalhesSecao);
...
const detalhesFornecedor = Array.isArray(dados.detalhesFornecedor)
  ? dados.detalhesFornecedor
  : [];

// TOTAL FORNECEDOR
const totalFornecedor = montarTotaisGenerico(
  detalhesFornecedor,
  'VLMETA',
  'PVENDA',
  'VLVENDA',
...
// ======================================
// 1) REALIZADO POR FORNECEDOR
// ======================================
const rowsFornecedor = detalhesFornecedor.map((item) => {
  const metaValor = Number(item.VLMETA || 0);
  const vlVendido = Number(item.PVENDA || 0);
  const vlFaturado = Number(item.VLVENDA || 0);
  const metaPos = Number(item.QTMETACLIPOS || 0);
...
rowsFornecedor.unshift({
  descricao: 'Total',
  metaValor: moeda(totalFornecedor.metaValor),
  vlVendido: moeda(totalFornecedor.vlVendido),
  vlFaturado: moeda(totalFornecedor.vlFaturado),
  percMeta: percentual(
    totalFornecedor.metaValor > 0
      ? (totalFornecedor.vlFaturado / totalFornecedor.metaValor) * 100
      : 0
  ),
  metaPos: numero(totalFornecedor.metaPos),
  realPos: numero(totalFornecedor.realPos),
  fatPos: numero(totalFornecedor.fatPos),
  percPos: percentual(
    totalFornecedor.metaPos > 0
      ? (totalFornecedor.fatPos / totalFornecedor.metaPos) * 100
      : 0
  )
});

y = addTable(doc, {
  title: 'REALIZADO POR FORNECEDOR',
  startY: y,
  fontSize: 7.5,
  rowHeight: 18,
  headerHeight: 20,
  columns: [
    { label: 'FORNECEDOR', key: 'descricao', width: 240 },
...
// ======================================
// 2) 70 / 80 / 90 POR FORNECEDOR
// ======================================
const totalFornecedor7090 = montarFaixas7090(
  totalFornecedor.metaValor,
  totalFornecedor.vlFaturado
);

const rowsFornecedor7090 = detalhesFornecedor.map((item) => {
...
rowsFornecedor7090.unshift({
  descricao: 'Total',
  metaValor: moeda(totalFornecedor7090.metaValor),
  vlFaturado: moeda(totalFornecedor7090.vlFaturado),
  v70: moeda(totalFornecedor7090.v70),
  v80: moeda(totalFornecedor7090.v80),
  v90: moeda(totalFornecedor7090.v90),
  f70: moeda(totalFornecedor7090.f70),
  f80: moeda(totalFornecedor7090.f80),
  f90: moeda(totalFornecedor7090.f90)
});
...
y = addTable(doc, {
  ...
  rows: rowsFornecedor7090
});

// ======================================
// 3) 100 / 110 / 120 POR FORNECEDOR
// ======================================
const totalFornecedor100120 = montarFaixas100120(
  totalFornecedor.metaValor,
  totalFornecedor.vlFaturado
);

const rowsFornecedor100120 = detalhesFornecedor.map((item) => {
...
rowsFornecedor100120.unshift({
  descricao: 'Total',
  metaValor: moeda(totalFornecedor100120.metaValor),
  vlFaturado: moeda(totalFornecedor100120.vlFaturado),
  v100: moeda(totalFornecedor100120.v100),
  v110: moeda(totalFornecedor100120.v110),
  v120: moeda(totalFornecedor100120.v120),
  f100: moeda(totalFornecedor100120.f100),
  f110: moeda(totalFornecedor100120.f110),
  f120: moeda(totalFornecedor100120.f120)
});

y = addTable(doc, {
  title: 'FAIXAS DE 100% / 110% / 120% - FORNECEDOR',
  startY: y,
  fontSize: 7.5,
  rowHeight: 18,
  headerHeight: 20,
  ...
  rows: rowsFornecedor100120
});

// ======================================
// 4) POSITIVAÇÃO POR FORNECEDOR
// ======================================
const totalPosFornecedor = detalhesFornecedor.reduce(
  (acc, item) => {
    acc.metaPos += Number(item.QTMETACLIPOS || 0);
    return acc;
  },
...
const totalPosFornecedorFaixas = montarFaixasPositivacao(totalPosFornecedor.metaPos);

const rowsPosFornecedor = detalhesFornecedor.map((item) => {
...
rowsPosFornecedor.unshift({
  descricao: 'Total',
  metaPos: numero(totalPosFornecedor.metaPos),
  p0a69: numero(totalPosFornecedorFaixas.p0a69),
  p70a79: numero(totalPosFornecedorFaixas.p70a79),
  p80a89: numero(totalPosFornecedorFaixas.p80a89),
  p90a99: numero(totalPosFornecedorFaixas.p90a99),
  p100a109: numero(totalPosFornecedorFaixas.p100a109),
  p110a119: numero(totalPosFornecedorFaixas.p110a119),
  pAcima120: numero(totalPosFornecedorFaixas.pAcima120)
});

y = addTable(doc, {
  title: 'POSITIVAÇÃO POR FORNECEDOR',
  startY: y,
  fontSize: 7.5,
  rowHeight: 18,
  headerHeight: 20,
  ...
  rows: rowsPosFornecedor
    ...
    const rows7090 = detalhesSecao.map((item) => {
      const fx = montarFaixas7090(item.VLMETA, item.VLVENDA);
      return {
        secao: item.SECAO,
        metaValor: moeda(fx.metaValor),
    ...
    const rows100120 = detalhesSecao.map((item) => {
      const fx = montarFaixas100120(item.VLMETA, item.VLVENDA);
      return {
        secao: item.SECAO,
        metaValor: moeda(fx.metaValor),
    ...
    const totalPositivacao = detalhesSecao.reduce(
  (acc, item) => {
    acc.metaPos += Number(item.QTMETACLIPOS || 0);
    return acc;
  },
...
const rowsPositivacao = detalhesSecao.map((item) => {
  const fx = montarFaixasPositivacao(item.QTMETACLIPOS);
